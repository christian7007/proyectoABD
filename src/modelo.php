...
function existe_grupo($conexion, $grupo){
   $stmt = $conexion->prepare("SELECT * FROM infogrupos WHERE nombre = ?");
...
function aniadir_grupos_gusto($conexion, $usuario, $genero){
   $stmt = $conexion->prepare("SELECT nombre FROM infogrupos WHERE genero = ? AND fecha_min <= (SELECT f_nacimiento FROM usuario WHERE nombre = ?) AND fecha_max >= (SELECT f_nacimiento FROM usuario WHERE nombre = ?)");
   $stmt->bind_param("sss", $genero, $usuario, $usuario);
   $stmt->execute();
   $stmt->bind_result($grupo);
   $grupos = array();
...
function insertar_usuarios_grupo($conexion, $fechamin, $fechamax, $genero, $nombre){
...
function crear_grupo($conexion, $fechamin, $fechamax, $genero, $nombre){
   $stmt = $conexion->prepare("INSERT INTO infogrupos VALUES (?, ?, ?, ?)");
   //almacenamos los datos al reves en la BD para dejar definido un intervalo y no que las fechas esten relacionadas con la edad que se introduce.
   $stmt->bind_param("ssss", $nombre, $fechamax, $fechamin, $genero);
   $stmt->execute();
   $stmt->close();
}
